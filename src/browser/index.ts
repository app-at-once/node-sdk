/**
 * AppAtOnce SDK - Browser Entry Point
 * 
...
const APPATONCE_BASE_URL = 'http://104.198.46.104:8091/api/v1';

// Check if we're in a browser environment
if (typeof window === 'undefined') {
  throw new Error('This module can only be imported in browser environments. For Node.js, use the main @appatonce/node-sdk import.');
}
...
/**
 * Load AppAtOnce Chatbot Widget
 * This is the ONLY function available in browser environments
 * 
 * @param apiKey - Your AppAtOnce API key
...
export function loadChatbot(apiKey: string, options: ChatbotOptions = {}): ChatbotWidget {
  ...
  if (!projectId) {
    throw new Error('Invalid API key - could not extract project ID');
  }
  ...
  container.id = 'appatonce-chatbot-container';
  ...
  const createChatWindow = () => {
    ...
    const sendMessage = async () => {
      ...
      try {
        // Send message to API
        const response = await fetch(`${APPATONCE_BASE_URL}/chatbot/send`, {
          method: 'POST',
          headers: {
        ...
        addMessage('assistant', data.answer || data.message || 'Sorry, I encountered an error.');
      } catch (error) {
...
function extractProjectId(apiKey: string): string | null {
  try {
    ...
    console.error('Failed to extract project ID from API key:', error);
    return null;
  }
}
