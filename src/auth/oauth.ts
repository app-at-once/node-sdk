...
export class OAuthModule {
  ...
  async refreshProviderToken(provider: OAuthProvider): Promise<OAuthTokenRefreshResponse> {
    try {
      const response = await this.httpClient.post(
        `/auth/oauth-connections/${provider}/refresh-token`
      );
  ...
  async trackProviderUsage(
    ...
    try {
      const response = await this.httpClient.post(
        `/auth/oauth-connections/${provider}/track-usage`,
        {
          action,
          metadata,
        }
  ...
  async getProviderConnectionStatus(provider: OAuthProvider): Promise<{
    ...
    try {
      const response = await this.httpClient.get(
        `/auth/oauth-connections/${provider}/is-connected`
      );
      return response.data;
    } catch (error) {
      return {
  ...
  async bulkDisconnectProviders(
    ...
    try {
      const response = await this.httpClient.post(
        '/auth/oauth-connections/bulk-disconnect',
        {
          providers,
        }
      );
