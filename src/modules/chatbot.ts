...
export class ChatbotModule {
  ...
  async render(options: ChatbotRenderOptions = {}): Promise<ChatbotWidget> {
    try {
      // Get API key and project ID from client config
      const apiKey = this.clientConfig.apiKey;
      const projectId = this.clientConfig.projectId || this.clientConfig.appId;
      
      if (!apiKey || !projectId) {
        throw new Error('API key and project ID are required for chatbot');
      }
      
      // Fetch chatbot configuration from server
      const config = await this.fetchConfig(projectId);
      if (!config.enabled) {
        throw new Error('Chatbot is not enabled for this project');
      }
      ...
      if (mode === 'iframe') {
        return this.renderIframe(options, apiKey, projectId);
      } else {
        return this.renderWidget(options, apiKey, projectId);
      }
    } catch (error) {
      if (options.onError) {
   ...
   * Fetch chatbot configuration from server
   */
  private async fetchConfig(projectId: string): Promise<ChatbotConfig> {
    const response = await this.httpClient.get(
      `/apps/${projectId}/chatbot/config`
    );
   ...
   */
  private renderIframe(options: ChatbotRenderOptions, apiKey: string, projectId: string): ChatbotWidget {
    ...
    const baseUrl = (this.clientConfig.baseUrl || 'https://api.appatonce.com').replace('/api/v1', '');
   ...
   */
  private renderWidget(options: ChatbotRenderOptions, apiKey: string, projectId: string): ChatbotWidget {
    // Create container
    const container = this.getContainer(options.container);
    ...
    this.setupRealtimeConnection(sessionId, messagesEl, options, projectId);
    ...
    const widget: ChatbotWidget = {
      ...
      setContext: (context) => { /* Store context */ },
      destroy: () => {
        widgetEl.remove();
        this.widget = null;
      }
