...
# This script patches the hardcoded URLs in the SDK from localhost to the public IP
...
if [ -f "src/constants.ts" ]; then
    ...
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' src/constants.ts
    rm src/constants.ts.bak
fi

if [ -f "src/browser/index.ts" ]; then
    echo "  Patching src/browser/index.ts..."
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' src/browser/index.ts
...
if [ -f "dist/constants.js" ]; then
    ...
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' dist/constants.js
...
if [ -f "dist/browser/index.js" ]; then
    ...
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' dist/browser/index.js
...
if [ -f "dist/constants.js.map" ]; then
    ...
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' dist/constants.js.map
    rm dist/constants.js.map.bak
fi

if [ -f "dist/browser/index.js.map" ]; then
    echo "  Patching dist/browser/index.js.map..."
    sed -i.bak 's|http://localhost:8091/api/v1|http://104.198.46.104:8091/api/v1|g' dist/browser/index.js.map
